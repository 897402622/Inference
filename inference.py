...
parser.add_argument("--dataset_dir", default="/home/Newdisk/wld/", type=str, help="train_dataset_dir")
parser.add_argument("--dataset_names", default=['PRCV'], nargs='+',
...
def test(): 
    ...
    net1.load_state_dict(
        torch.load("/home/Newdisk/wld/BasicIRSTD-main/log/PRCV/ISTDU-Net_240.pth(1).tar")['state_dict'])
    net1.eval()

    net2 = Net(model_name='ISTDU-Net', mode='test').cuda()
    net2.load_state_dict(torch.load("/home/Newdisk/wld/BasicIRSTD-main/log/PRCV/ISTDU-Net_140.pth.tar")['state_dict'])
    net2.eval()

    net3 = Net(model_name='SCTransNet', mode='test').cuda()
    net3.load_state_dict(torch.load("/home/Newdisk/wld/BasicIRSTD-main/log/PRCV/SCTransNet_300.pth.tar")['state_dict'])
    ...
    net4.load_state_dict(torch.load("/home/Newdisk/wld/BasicIRSTD-main/log/PRCV/DNANet_70.pth.tar")['state_dict'])
    net4.eval()

    net5 = Net(model_name='RDIAN', mode='test').cuda()
    net5.load_state_dict(torch.load("/home/Newdisk/wld/BasicIRSTD-main/log/PRCV/RDIAN_220.pth.tar")['state_dict'])
    net5.eval()
    # try:
